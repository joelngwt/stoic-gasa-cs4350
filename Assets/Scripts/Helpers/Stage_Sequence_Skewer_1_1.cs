...
public class Stage_Sequence_Skewer_1_1 : Stage_Sequence_Helper {
	...
	public override void process_update () 	{
		...
		if(sequence_phase_index == 0)
		{
			 ...
			 * Initialize 
			 * */
			Camera.main.GetComponent<World_Object_Movement_Helper>().add_movement_and_lookAt_task(new Vector3( 0.0F, 23.0F, -12.0F), 
			                                                                                      World_Object_Movement_Helper.PLAYER_WORLD_OBJECT_MOVEMENT_SPEED_POSSESSION, 
			                                                                                      new Vector3(0.0f, 67f, -122f), 
			                                                                                      Vector3.up, 
		...
		{
			if(Vector3.Distance(Camera.main.transform.position, new Vector3( 0.0F, 23.0F, -12.0F)) <= 1.00F)
			{
		...
		else if(sequence_phase_index == 2)
		{
			 ...
			 * */
			possession_cube = spawn_possession_cube(new Vector3( 0.0f, 67f, -122f));
			possession_cube_script = possession_cube.AddComponent("Possession_Cube") as Possession_Cube;
			possession_cube_script.lookAt_position = new Vector3( 0.0F, 23.0F, -12.0F);
		...
		else if(sequence_phase_index == 4)
		{
			...
			possession_cube = spawn_possession_cube(new Vector3( 0.0F, 23.0F, -12.0F));
			possession_cube_script = possession_cube.AddComponent("Possession_Cube") as Possession_Cube;
			possession_cube_script.lookAt_position = new Vector3( 0.0f, 67f, -122f);
			possession_cube_script.up_vector = Vector3.up;
		...
		else if(sequence_phase_index == 8)
		{
			...
			Camera.main.GetComponent<World_Object_Movement_Helper>().add_lookAt_task(new Vector3(0.0F, 131.0F, 10.0F), 
			                                                                         Vector3.up, 
			                                                                         World_Object_Movement_Helper.PLAYER_WORLD_OBJECT_ROTATION_SPEED_DEFAULT);
		...
		else if(sequence_phase_index == 9)
		{
			...
			possession_cube = spawn_possession_cube(new Vector3( 0.0F, 135.0F, 8.0F));
			possession_cube_script = possession_cube.AddComponent("Possession_Cube") as Possession_Cube;
			possession_cube_script.lookAt_position = new Vector3( 0.0F, 131.0F, -113.0F);
